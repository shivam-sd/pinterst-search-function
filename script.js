...
function showTheImg(){
    ...
    // console.log(clutter);
    let showImg = document.getElementById("img-container");
    showImg.innerHTML = clutter;
}
...
function searchcard(){
    search.addEventListener("input",()=>{
        ...
        var clutter = "";
        const filterdArray = arr.filter(obj => obj.name.toLowerCase().startsWith(search.value));
        console.log(filterdArray);
        filterdArray.forEach((ob) => {
            clutter += `  <div class="data">
            <h3 class="data1">${ob.name}</h3>
                </div>`;
                console.log(ob.name);
            })
            document.querySelector("#show-data").style.display = 'inline-block';
            document.querySelector("#show-data").innerHTML = clutter;
    })
}
